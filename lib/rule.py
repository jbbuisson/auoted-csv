...
class Rule:
    """Rule objects standardize files according to the desired delimiter and column count"""
    def __init__(
            self,
            delimiter: str,
            column_count: int):
    ...
    def standardize_file(
        ...
        with open(output_directory / file.name, "w", encoding='utf-8') as csv_output_file:
            Rule._write_rows(csv_output_file, input_rows)
    ...
    def _read_rows(self, filename, csv_input_file) -> list:
        ...
        for row in data_reader:
            ...
            if len(row) != self.column_count:
                ...
                logging.error(
                    f"{filename} - line {row_number} - Expected {self.column_count} column(s), but found {len(row)}"
                )
    ...
    @staticmethod
    def _write_rows(output_csv_file, input_rows):
